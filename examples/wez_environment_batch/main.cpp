...
int main() {
  ...
  const uint32_t EGO_SDIM =
      5; // 4 floats + 1 bool, struct packing makes this 5 floats
  const uint32_t CDIM = 2;

  // 4 floats + 1 bool + 1 uint16_t -> Packed into 5 'floats' worth of space by
  // struct rules
  const uint32_t ADVERSARY_SDIM = 5;

  const uint32_t SDIM = EGO_SDIM + N_ADVERSARIES * ADVERSARY_SDIM;
  static_assert(sizeof(AgentState) == SDIM * sizeof(float),
                "AgentState layout mismatch");
  ...
  params.model_params = {reinterpret_cast<uint8_t *>(&m_params),
                         sizeof(ModelParams)};
  params.cost_params = {reinterpret_cast<uint8_t *>(&c_params),
                        sizeof(CostParams)};
  ...
  std::cout << "Running " << N_AGENTS << " batched agents for " << N_STEPS
            << " steps toward (" << c_params.x_goal << ", " << c_params.y_goal
            << ")...\n";

  auto t0 = std::chrono::high_resolution_clock::now();

  if (!driver.simulate_batch(N_AGENTS, N_STEPS, x0_packed.data(), results,
                             &error)) {
  ...
  for (uint32_t i = 0; i < N_ADVERSARIES; ++i) {
    csv << ",x_adv_" << i + 1 << ",y_adv_" << i + 1 << ",th_adv_" << i + 1
        << ",v_adv_" << i + 1;
  }
  csv << "\n";
  ...
  for (uint32_t a = 0; a < N_AGENTS; ++a) {
    ...
    for (uint32_t step = 0; step < N_STEPS; ++step) {
      ...
      csv << a << "," << step << "," << x << "," << y << "," << theta << ","
          << v << "," << a_cmd << "," << omega_cmd << "," << captured << ","
          << best_cost;

      for (uint32_t i = 0; i < N_ADVERSARIES; ++i) {
        uint32_t adv_base = s_base + EGO_SDIM + i * ADVERSARY_SDIM;
        csv << "," << states_out[adv_base + 0] << ","
            << states_out[adv_base + 1] << "," << states_out[adv_base + 2]
  ...
  // Copy last state to initial state for next iteration
  std::copy(states_out.begin() + (N_STEPS - 1) * N_AGENTS * SDIM,
            states_out.begin() + N_STEPS * N_AGENTS * SDIM, x0_packed.begin());

  // Update the goal position
  c_params.x_goal = 0.0f;
  c_params.y_goal = 10.0f;
  ...
  csv.close();
  //   std::cout << reached_goal << "/" << N_AGENTS << " agents reached the
  //   goal\n";
  std::cout << "Trajectory data logged to trajectory_batch.csv\n";
  return 0;
}
